...
BEGIN
UPDATE ice_cream
SET category_id = category_id1, name = name1, discount = discount1, ice_description = ice_description1, price = price1, sold_quantity = sold_quantity1, src = scr1
...
DELIMITER //
CREATE PROCEDURE search_User(in name varchar(255))
BEGIN 
select * from users where users.user_name=name;
END //
DELIMITER ;
